...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-slate-950 via-slate-900 to-black text-slate-100">
      ...
      <main className="max-w-6xl mx-auto px-4 pb-24 pt-6 grid gap-6">
        {/* Row 1: Side-by-side images */}
        <div className="grid gap-6 md:grid-cols-2">
          {/* Camera Card */}
          <motion.div layout className="rounded-2xl border border-white/10 bg-slate-900/50 shadow-xl overflow-hidden">
            <div className="p-4 flex items-center gap-2 border-b border-white/10">
              <Aperture className="w-5 h-5" />
              <span className="font-medium">Camera</span>
              <div className="ml-auto flex items-center gap-2">
                {stream ? (
                  <button onClick={stopCamera} className="px-3 py-1.5 text-sm rounded-xl bg-red-600/80 hover:bg-red-600 transition">
                    Stop
                  </button>
                ) : (
                  <button onClick={startCamera} className="px-3 py-1.5 text-sm rounded-xl bg-emerald-600/80 hover:bg-emerald-600 transition" disabled={isOpening}>
            ...
            <div className="relative aspect-[3/4] bg-black">
              {!captured ? (
                <>
                  <video ref={videoRef} playsInline muted className="w-full h-full object-cover" autoPlay />
                  {!stream && (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center opacity-80 px-6">
                        <ImageIcon className="w-10 h-10 mx-auto mb-2" />
                        <p className="text-sm">Tap “Open Camera” to start a live preview, then hit the shutter to capture.</p>
                        {hasPermission === false && (
                          <p className="text-xs text-red-300 mt-2">Permission denied. Enable camera access in your browser settings.</p>
                        )}
                      </div>
                    </div>
                  )}
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 flex items-center justify-center gap-3 bg-gradient-to-t from-black/70 to-transparent">
                {!captured ? (
                  <button onClick={capturePhoto} className="rounded-full p-4 bg-white/90 hover:bg-white text-black shadow-lg" title="Capture" disabled={!stream}>
                    <Wand2 className="w-6 h-6" />
                  </button>
                ) : (
                  <div className="flex gap-2">
                    <button onClick={retake} className="px-3 py-1.5 rounded-xl bg-slate-100 text-slate-900 hover:bg-white">
                      <RefreshCcw className="w-4 h-4 inline -mt-0.5 mr-1" /> Retake
                    </button>
                    <button onClick={() => setCaptured(null)} className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-white/10">
          ...
          <motion.div layout className="rounded-2xl border border-white/10 bg-slate-900/50 shadow-xl overflow-hidden">
            ...
            <div className="relative aspect-[3/4] bg-slate-950 flex items-center justify-center">
              {result?.imageDataUrl ? (
                <img src={result.imageDataUrl} alt="Edited" className="w-full h-full object-cover" />
              ) : (
                <div className="text-center px-6 opacity-80">
                  <p className="text-sm">Your customized image will appear here.</p>
                  <p className="text-xs mt-1">Capture a selfie on the left, write a prompt below, and press Submit.</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        <div className="grid gap-6">
          {/* Prompt */}
          <motion.div layout className="rounded-2xl border border-white/10 bg-slate-900/50 shadow-xl">
            <div className="p-4 border-b border-white/10 flex items-center gap-2">
            ...
            <div className="p-4">
              <div className="flex flex-wrap gap-2 mb-3">
                {SUGGESTED_PROMPTS.map((s) => (
                  <button key={s} onClick={() => useSuggestion(s)} className="text-xs px-3 py-1.5 rounded-full border border-white/10 hover:border-white/20 bg-slate-800/60 hover:bg-slate-800 transition">
                    {s}
                  </button>
                ))}
              </div>
              <label className="block text-sm mb-2 opacity-80">Describe the edit you want</label>
              <textarea
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="e.g., Apply police uniform with badge; keep glasses; match lighting"
                className="w-full min-h-[120px] rounded-xl bg-slate-800/80 border border-white/10 p-3 outline-none focus:ring-2 focus:ring-emerald-500/50"
              />
              <div className="mt-3 flex justify-end gap-2">
                ...
                >Submit</button>
                <button
                  onClick={() => { setPrompt(""); setCaptured(null); setResult(null); }}
                  className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-white/10"
                >Reset</button>
              </div>

              {error && (
                <div className="mt-3 text-sm text-red-300 bg-red-950/40 border border-red-900/40 rounded-xl p-3">{error}</div>
              )}
            </div>
          </motion.div>
        </div>
      ...
      <footer className="max-w-6xl mx-auto px-4 pb-10 text-center opacity-60 text-xs">
        Built with getUserMedia • Images are sent to your backend only for each request (never stored).
      </footer>
    </div>
  );
}
